...
public class ISO8601Utils {
    ...
    private static int parseInt(String value, int beginIndex, int endIndex) throws NumberFormatException {
        int digit;
        if (beginIndex < 0) throw new NumberFormatException(value);
        if (endIndex > value.length()) throw new NumberFormatException(value);
        ...
        try {
            ...
            if (timezoneIndicator == 'Z') {
                ...
                if (timezoneIndicator != '+') {
                    if (timezoneIndicator != '-') throw new IndexOutOfBoundsException("Invalid time zone indicator '" + timezoneIndicator + "'");
                }
                timezoneOffset = (timezoneOffset = date.substring(offset)).length() >= 5 ? timezoneOffset : timezoneOffset + "00";
