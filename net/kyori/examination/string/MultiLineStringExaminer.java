 ...
 *  net.kyori.examination.string.MultiLineStringExaminer$Instances
...
extends AbstractExaminer<Stream<String>> {
    ...
    private static Stream<String> association(List<String> left, String middle, List<String> right) {
        ...
        String leftPad = lefts < 2 ? "" : Strings.repeat((String)" ", (int)leftWidth);
        String middlePad = lefts < 2 ? "" : Strings.repeat((String)" ", (int)middle.length());
        ArrayList<String> result = new ArrayList<String>(height);
        int i = 0;
        while (i < height) {
            String l = i < lefts ? Strings.padEnd((String)left.get(i), (int)leftWidth, (char)' ') : leftPad;
    ...
    protected <T> Stream<String> stream(@NotNull Stream<T> stream) {
        return this.arrayLike(stream.map(arg_0 -> ((MultiLineStringExaminer)this).examine(arg_0)));
    }
