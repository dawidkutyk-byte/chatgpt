...
public class SocketChannelIOHelper {
    public static boolean readMore(ByteBuffer buf, WebSocketImpl ws, WrappedByteChannel channel) throws IOException {
        ...
        buf.flip();
        if (read != -1) return channel.isNeedRead();
        ws.eot();
        return false;
    }
    ...
    public static boolean batch(WebSocketImpl ws, ByteChannel sockchannel) throws IOException {
        ...
        if (!ws.outQueue.isEmpty() || !ws.isFlushAndClose() || ws.getDraft() == null || ws.getDraft().getRole() == null || ws.getDraft().getRole() != Role.SERVER) return c == null || !((WrappedByteChannel)sockchannel).isNeedWrite();
        ws.closeConnection();
