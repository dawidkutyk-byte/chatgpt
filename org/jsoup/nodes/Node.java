/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  7\u015aCz
 *  org.jsoup.helper.Validate
 *  org.jsoup.internal.StringUtil
 *  org.jsoup.nodes.Attributes
 *  org.jsoup.nodes.Document
 *  org.jsoup.nodes.Document$OutputSettings
 *  org.jsoup.nodes.Element
 *  org.jsoup.nodes.Node$OuterHtmlVisitor
 *  org.jsoup.nodes.NodeUtils
 *  org.jsoup.nodes.Range
 *  org.jsoup.nodes.TextNode
 *  org.jsoup.select.NodeFilter
 *  org.jsoup.select.NodeTraversor
 *  org.jsoup.select.NodeVisitor
 */
package org.jsoup.nodes;

import java.io.IOException;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.Collections;
import java.util.Iterator;
import java.util.LinkedList;
import java.util.List;
import java.util.function.Consumer;
import java.util.stream.Stream;
import org.jsoup.helper.Validate;
import org.jsoup.internal.StringUtil;
import org.jsoup.nodes.Attributes;
import org.jsoup.nodes.Document;
import org.jsoup.nodes.Element;
import org.jsoup.nodes.Node;
import org.jsoup.nodes.NodeUtils;
import org.jsoup.nodes.Range;
import org.jsoup.nodes.TextNode;
import org.jsoup.select.NodeFilter;
import org.jsoup.select.NodeTraversor;
import org.jsoup.select.NodeVisitor;

public abstract class Node
implements Cloneable {
    static final String EmptyString = "";
    @7\u015aCz Node parentNode;
    static final List<Node> EmptyNodes = Collections.emptyList();
    int siblingIndex;

    public List<Node> siblingNodes() {
        if (this.parentNode == null) {
            return Collections.emptyList();
        }
        List<Node> nodes = this.parentNode.ensureChildNodes();
        ArrayList<Node> siblings = new ArrayList<Node>(nodes.size() - 1);
        Iterator<Node> iterator = nodes.iterator();
        while (iterator.hasNext()) {
            Node node = iterator.next();
            if (node == this) continue;
            siblings.add(node);
        }
        return siblings;
    }

    protected abstract boolean hasAttributes();

    public String outerHtml() {
        StringBuilder accum = StringUtil.borrowBuilder();
        this.outerHtml(accum);
        return StringUtil.releaseBuilder((StringBuilder)accum);
    }

    public Node childNode(int index) {
        return this.ensureChildNodes().get(index);
    }

    public abstract int childNodeSize();

    public @7\u015aCz Node lastChild() {
        int size = this.childNodeSize();
        if (size == 0) {
            return null;
        }
        List<Node> children = this.ensureChildNodes();
        return children.get(size - 1);
    }

    public final @7\u015aCz Node parentNode() {
        return this.parentNode;
    }

    public abstract String nodeName();

    protected void addChildren(int index, Node ... children) {
        Validate.notNull((Object)children);
        if (children.length == 0) {
            return;
        }
        List<Node> nodes = this.ensureChildNodes();
        Node firstParent = children[0].parent();
        if (firstParent != null && firstParent.childNodeSize() == children.length) {
            boolean sameList = true;
            List<Node> firstParentNodes = firstParent.ensureChildNodes();
            int i = children.length;
            while (i-- > 0) {
                if (children[i] == firstParentNodes.get(i)) continue;
                sameList = false;
                break;
            }
            if (sameList) {
                boolean wasEmpty = this.childNodeSize() == 0;
                firstParent.empty();
                nodes.addAll(index, Arrays.asList(children));
                i = children.length;
                while (i-- > 0) {
                    children[i].parentNode = this;
                }
                if (wasEmpty) {
                    if (children[0].siblingIndex == 0) return;
                }
                this.reindexChildren(index);
                return;
            }
        }
        Validate.noNullElements((Object[])children);
        Node[] nodeArray = children;
        int n = nodeArray.length;
        int n2 = 0;
        while (true) {
            if (n2 >= n) {
                nodes.addAll(index, Arrays.asList(children));
                this.reindexChildren(index);
                return;
            }
            Node child = nodeArray[n2];
            this.reparentChild(child);
            ++n2;
        }
    }

    private Element getDeepChild(Element el) {
        Element child = el.firstElementChild();
        while (child != null) {
            el = child;
            child = child.firstElementChild();
        }
        return el;
    }

    public int siblingIndex() {
        return this.siblingIndex;
    }

    public Node traverse(NodeVisitor nodeVisitor) {
        Validate.notNull((Object)nodeVisitor);
        NodeTraversor.traverse((NodeVisitor)nodeVisitor, (Node)this);
        return this;
    }

    public boolean hasAttr(String attributeKey) {
        Validate.notNull((Object)attributeKey);
        if (!this.hasAttributes()) {
            return false;
        }
        if (!attributeKey.startsWith("abs:")) return this.attributes().hasKeyIgnoreCase(attributeKey);
        String key = attributeKey.substring("abs:".length());
        if (!this.attributes().hasKeyIgnoreCase(key)) return this.attributes().hasKeyIgnoreCase(attributeKey);
        if (this.absUrl(key).isEmpty()) return this.attributes().hasKeyIgnoreCase(attributeKey);
        return true;
    }

    public Node clearAttributes() {
        if (!this.hasAttributes()) return this;
        Iterator it = this.attributes().iterator();
        while (it.hasNext()) {
            it.next();
            it.remove();
        }
        return this;
    }

    protected void setSiblingIndex(int siblingIndex) {
        this.siblingIndex = siblingIndex;
    }

    public boolean nameIs(String normalName) {
        return this.normalName().equals(normalName);
    }

    private void addSiblingHtml(int index, String html) {
        Validate.notNull((Object)html);
        Validate.notNull((Object)this.parentNode);
        Element context = this.parent() instanceof Element ? (Element)this.parent() : null;
        List nodes = NodeUtils.parser((Node)this).parseFragmentInput(html, context, this.baseUri());
        this.parentNode.addChildren(index, nodes.toArray(new Node[0]));
    }

    public List<Node> childNodes() {
        if (this.childNodeSize() == 0) {
            return EmptyNodes;
        }
        List<Node> children = this.ensureChildNodes();
        ArrayList<Node> rewrap = new ArrayList<Node>(children.size());
        rewrap.addAll(children);
        return Collections.unmodifiableList(rewrap);
    }

    protected Node doClone(@7\u015aCz Node parent) {
        Node clone;
        try {
            clone = (Node)super.clone();
        }
        catch (CloneNotSupportedException e) {
            throw new RuntimeException(e);
        }
        clone.parentNode = parent;
        clone.siblingIndex = parent == null ? 0 : this.siblingIndex;
        if (parent != null) return clone;
        if (this instanceof Document) return clone;
        Document doc = this.ownerDocument();
        if (doc == null) return clone;
        Document docClone = doc.shallowClone();
        clone.parentNode = docClone;
        docClone.ensureChildNodes().add(clone);
        return clone;
    }

    final boolean isEffectivelyFirst() {
        if (this.siblingIndex == 0) {
            return true;
        }
        if (this.siblingIndex != 1) return false;
        Node prev = this.previousSibling();
        return prev instanceof TextNode && ((TextNode)prev).isBlank();
    }

    public Node forEachNode(Consumer<? super Node> action) {
        Validate.notNull(action);
        this.nodeStream().forEach(action);
        return this;
    }

    abstract void outerHtmlTail(Appendable var1, int var2, Document.OutputSettings var3) throws IOException;

    public boolean hasSameValue(@7\u015aCz Object o) {
        if (this == o) {
            return true;
        }
        if (o == null) return false;
        if (this.getClass() == o.getClass()) return this.outerHtml().equals(((Node)o).outerHtml());
        return false;
    }

    public Node shallowClone() {
        return this.doClone(null);
    }

    public String attr(String attributeKey) {
        Validate.notNull((Object)attributeKey);
        if (!this.hasAttributes()) {
            return EmptyString;
        }
        String val = this.attributes().getIgnoreCase(attributeKey);
        if (val.length() > 0) {
            return val;
        }
        if (!attributeKey.startsWith("abs:")) return EmptyString;
        return this.absUrl(attributeKey.substring("abs:".length()));
    }

    public Node removeAttr(String attributeKey) {
        Validate.notNull((Object)attributeKey);
        if (!this.hasAttributes()) return this;
        this.attributes().removeIgnoreCase(attributeKey);
        return this;
    }

    public void remove() {
        if (this.parentNode == null) return;
        this.parentNode.removeChild(this);
    }

    protected abstract List<Node> ensureChildNodes();

    protected void setParentNode(Node parentNode) {
        Validate.notNull((Object)parentNode);
        if (this.parentNode != null) {
            this.parentNode.removeChild(this);
        }
        this.parentNode = parentNode;
    }

    protected void addChildren(Node ... children) {
        List<Node> nodes = this.ensureChildNodes();
        Node[] nodeArray = children;
        int n = nodeArray.length;
        int n2 = 0;
        while (n2 < n) {
            Node child = nodeArray[n2];
            this.reparentChild(child);
            nodes.add(child);
            child.setSiblingIndex(nodes.size() - 1);
            ++n2;
        }
    }

    public <T extends Appendable> T html(T appendable) {
        this.outerHtml(appendable);
        return appendable;
    }

    public Node before(String html) {
        this.addSiblingHtml(this.siblingIndex, html);
        return this;
    }

    public @7\u015aCz Node parent() {
        return this.parentNode;
    }

    public Node wrap(String html) {
        Validate.notEmpty((String)html);
        Element context = this.parentNode != null && this.parentNode instanceof Element ? (Element)this.parentNode : (this instanceof Element ? (Element)this : null);
        List wrapChildren = NodeUtils.parser((Node)this).parseFragmentInput(html, context, this.baseUri());
        Node wrapNode = (Node)wrapChildren.get(0);
        if (!(wrapNode instanceof Element)) {
            return this;
        }
        Element wrap = (Element)wrapNode;
        Element deepest = this.getDeepChild(wrap);
        if (this.parentNode != null) {
            this.parentNode.replaceChild(this, (Node)wrap);
        }
        deepest.addChildren(new Node[]{this});
        if (wrapChildren.size() <= 0) return this;
        int i = 0;
        while (i < wrapChildren.size()) {
            Node remainder = (Node)wrapChildren.get(i);
            if (wrap != remainder) {
                if (remainder.parentNode != null) {
                    remainder.parentNode.removeChild(remainder);
                }
                wrap.after(remainder);
            }
            ++i;
        }
        return this;
    }

    public @7\u015aCz Node firstChild() {
        if (this.childNodeSize() != 0) return this.ensureChildNodes().get(0);
        return null;
    }

    public @7\u015aCz Document ownerDocument() {
        Node root = this.root();
        return root instanceof Document ? (Document)root : null;
    }

    protected void outerHtml(Appendable accum) {
        NodeTraversor.traverse((NodeVisitor)new OuterHtmlVisitor(accum, NodeUtils.outputSettings((Node)this)), (Node)this);
    }

    protected void reparentChild(Node child) {
        child.setParentNode(this);
    }

    public abstract Node empty();

    public boolean parentElementIs(String normalName, String namespace) {
        return this.parentNode != null && this.parentNode instanceof Element && ((Element)this.parentNode).elementIs(normalName, namespace);
    }

    public @7\u015aCz Node unwrap() {
        Validate.notNull((Object)this.parentNode);
        Node firstChild = this.firstChild();
        this.parentNode.addChildren(this.siblingIndex, this.childNodesAsArray());
        this.remove();
        return firstChild;
    }

    public abstract String baseUri();

    public Node after(Node node) {
        Validate.notNull((Object)node);
        Validate.notNull((Object)this.parentNode);
        if (node.parentNode == this.parentNode) {
            node.remove();
        }
        this.parentNode.addChildren(this.siblingIndex + 1, node);
        return this;
    }

    protected Node[] childNodesAsArray() {
        return this.ensureChildNodes().toArray(new Node[0]);
    }

    protected void removeChild(Node out) {
        Validate.isTrue((out.parentNode == this ? 1 : 0) != 0);
        int index = out.siblingIndex;
        this.ensureChildNodes().remove(index);
        this.reindexChildren(index);
        out.parentNode = null;
    }

    public String toString() {
        return this.outerHtml();
    }

    protected void indent(Appendable accum, int depth, Document.OutputSettings out) throws IOException {
        accum.append('\n').append(StringUtil.padding((int)(depth * out.indentAmount()), (int)out.maxPaddingWidth()));
    }

    public @7\u015aCz Node previousSibling() {
        if (this.parentNode == null) {
            return null;
        }
        if (this.siblingIndex <= 0) return null;
        return this.parentNode.ensureChildNodes().get(this.siblingIndex - 1);
    }

    public Node before(Node node) {
        Validate.notNull((Object)node);
        Validate.notNull((Object)this.parentNode);
        if (node.parentNode == this.parentNode) {
            node.remove();
        }
        this.parentNode.addChildren(this.siblingIndex, node);
        return this;
    }

    public void replaceWith(Node in) {
        Validate.notNull((Object)in);
        Validate.notNull((Object)this.parentNode);
        this.parentNode.replaceChild(this, in);
    }

    public Node attr(String attributeKey, String attributeValue) {
        attributeKey = NodeUtils.parser((Node)this).settings().normalizeAttribute(attributeKey);
        this.attributes().putIgnoreCase(attributeKey, attributeValue);
        return this;
    }

    public @7\u015aCz Node nextSibling() {
        if (this.parentNode == null) {
            return null;
        }
        List<Node> siblings = this.parentNode.ensureChildNodes();
        int index = this.siblingIndex + 1;
        if (siblings.size() <= index) return null;
        return siblings.get(index);
    }

    public Node root() {
        Node node = this;
        while (node.parentNode != null) {
            node = node.parentNode;
        }
        return node;
    }

    public <T extends Node> Stream<T> nodeStream(Class<T> type) {
        return NodeUtils.stream((Node)this, type);
    }

    public Node filter(NodeFilter nodeFilter) {
        Validate.notNull((Object)nodeFilter);
        NodeTraversor.filter((NodeFilter)nodeFilter, (Node)this);
        return this;
    }

    void nodelistChanged() {
    }

    public int attributesSize() {
        return this.hasAttributes() ? this.attributes().size() : 0;
    }

    public Range sourceRange() {
        return Range.of((Node)this, (boolean)true);
    }

    public int hashCode() {
        return super.hashCode();
    }

    private void reindexChildren(int start) {
        int size = this.childNodeSize();
        if (size == 0) {
            return;
        }
        List<Node> childNodes = this.ensureChildNodes();
        int i = start;
        while (i < size) {
            childNodes.get(i).setSiblingIndex(i);
            ++i;
        }
    }

    protected Node() {
    }

    public boolean equals(@7\u015aCz Object o) {
        return this == o;
    }

    public List<Node> childNodesCopy() {
        List<Node> nodes = this.ensureChildNodes();
        ArrayList<Node> children = new ArrayList<Node>(nodes.size());
        Iterator<Node> iterator = nodes.iterator();
        while (iterator.hasNext()) {
            Node node = iterator.next();
            children.add((Node)node.clone());
        }
        return children;
    }

    abstract void outerHtmlHead(Appendable var1, int var2, Document.OutputSettings var3) throws IOException;

    public boolean parentNameIs(String normalName) {
        return this.parentNode != null && this.parentNode.normalName().equals(normalName);
    }

    public Node after(String html) {
        this.addSiblingHtml(this.siblingIndex + 1, html);
        return this;
    }

    protected abstract void doSetBaseUri(String var1);

    public Node clone() {
        Node thisClone = this.doClone(null);
        LinkedList<Node> nodesToProcess = new LinkedList<Node>();
        nodesToProcess.add(thisClone);
        block0: while (!nodesToProcess.isEmpty()) {
            Node currParent = (Node)nodesToProcess.remove();
            int size = currParent.childNodeSize();
            int i = 0;
            while (true) {
                if (i >= size) continue block0;
                List<Node> childNodes = currParent.ensureChildNodes();
                Node childClone = childNodes.get(i).doClone(currParent);
                childNodes.set(i, childClone);
                nodesToProcess.add(childClone);
                ++i;
            }
            break;
        }
        return thisClone;
    }

    public abstract Attributes attributes();

    public String normalName() {
        return this.nodeName();
    }

    protected void replaceChild(Node out, Node in) {
        Validate.isTrue((out.parentNode == this ? 1 : 0) != 0);
        Validate.notNull((Object)in);
        if (out == in) {
            return;
        }
        if (in.parentNode != null) {
            in.parentNode.removeChild(in);
        }
        int index = out.siblingIndex;
        this.ensureChildNodes().set(index, in);
        in.parentNode = this;
        in.setSiblingIndex(index);
        out.parentNode = null;
    }

    public boolean hasParent() {
        return this.parentNode != null;
    }

    public void setBaseUri(String baseUri) {
        Validate.notNull((Object)baseUri);
        this.doSetBaseUri(baseUri);
    }

    public String absUrl(String attributeKey) {
        Validate.notEmpty((String)attributeKey);
        if (!this.hasAttributes()) return EmptyString;
        if (this.attributes().hasKeyIgnoreCase(attributeKey)) return StringUtil.resolve((String)this.baseUri(), (String)this.attributes().getIgnoreCase(attributeKey));
        return EmptyString;
    }

    public Stream<Node> nodeStream() {
        return NodeUtils.stream((Node)this, Node.class);
    }
}
