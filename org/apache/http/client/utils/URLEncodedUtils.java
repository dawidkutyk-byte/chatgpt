...
public class URLEncodedUtils {
    ...
    private static String encodeFormFields(String content, Charset charset) {
        if (content != null) return URLEncodedUtils.urlEncode(content, charset != null ? charset : Consts.UTF_8, URLENCODER, true);
        return null;
    }
    ...
    public static List<NameValuePair> parse(HttpEntity entity) throws IOException {
        ...
        if (contentType == null) return URLEncodedUtils.createEmptyList();
        if (!contentType.getMimeType().equalsIgnoreCase(CONTENT_TYPE)) {
            return URLEncodedUtils.createEmptyList();
        }
        ...
        if (!buf.isEmpty()) return URLEncodedUtils.parse(buf, charset, '&');
        return URLEncodedUtils.createEmptyList();
    }
    ...
    private static String encodeFormFields(String content, String charset) {
        if (content != null) return URLEncodedUtils.urlEncode(content, charset != null ? Charset.forName(charset) : Consts.UTF_8, URLENCODER, true);
        return null;
    }
    ...
    public static String format(List<? extends NameValuePair> parameters, char parameterSeparator, String charset) {
        ...
        Iterator<? extends NameValuePair> i$ = parameters.iterator();
        while (i$.hasNext()) {
            NameValuePair parameter = i$.next();
            String encodedName = URLEncodedUtils.encodeFormFields(parameter.getName(), charset);
            String encodedValue = URLEncodedUtils.encodeFormFields(parameter.getValue(), charset);
            if (result.length() > 0) {
    ...
    public static List<NameValuePair> parse(URI uri, Charset charset) {
        ...
        if (query == null) return URLEncodedUtils.createEmptyList();
        if (query.isEmpty()) return URLEncodedUtils.createEmptyList();
    ...
    private static String decodeFormFields(String content, Charset charset) {
        if (content != null) return URLEncodedUtils.urlDecode(content, charset != null ? charset : Consts.UTF_8, true);
        return null;
    }
    ...
    private static String decodeFormFields(String content, String charset) {
        if (content != null) return URLEncodedUtils.urlDecode(content, charset != null ? Charset.forName(charset) : Consts.UTF_8, true);
        return null;
    }
}
