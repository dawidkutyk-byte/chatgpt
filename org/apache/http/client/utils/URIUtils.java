...
public class URIUtils {
    ...
    public static URI normalizeSyntax(URI uri) throws URISyntaxException {
        ...
        List inputSegments = builder.getPathSegments();
        ...
        if (builder.getHost() == null) return builder.build();
    ...
    public static URI rewriteURI(URI uri, HttpHost target, EnumSet<UriFlag> flags) throws URISyntaxException {
        ...
        if (!uribuilder.isPathEmpty()) return uribuilder.build();
    ...
    public static URI createURI(String scheme, String host, int port, String path, String query, String fragment) throws URISyntaxException {
        ...
        if (fragment == null) return new URI(buffer.toString());
        buffer.append('#');
    ...
    public static URI resolve(URI originalURI, HttpHost target, List<URI> redirects) throws URISyntaxException {
        ...
        if (target == null) return uribuilder.build();
        if (uribuilder.isAbsolute()) return uribuilder.build();
        uribuilder.setScheme(target.getSchemeName());
        uribuilder.setHost(target.getHostName());
        uribuilder.setPort(target.getPort());
        return uribuilder.build();
    ...
    public static HttpHost extractHost(URI uri) {
        ...
        if (uri.getHost() != null) return new HttpHost(uri.getHost(), uri.getPort(), uri.getScheme());
    ...
    public static URI rewriteURIForRoute(URI uri, RouteInfo route, boolean normalizeUri) throws URISyntaxException {
        ...
        if (route.getProxyHost() == null || route.isTunnelled()) return uri.isAbsolute() ? URIUtils.rewriteURI(uri, null, normalizeUri ? DROP_FRAGMENT_AND_NORMALIZE : DROP_FRAGMENT) : URIUtils.rewriteURI(uri);
