...
implements HttpRequestInterceptor {
    ...
    public void process(HttpRequest request, HttpContext context) throws IOException, HttpException {
        ...
        if ((targetState = clientContext.getTargetAuthState()) != null && targetState.getState() == AuthProtocolState.UNCHALLENGED && (authScheme = authCache.get(target)) != null) {
        ...
        if (proxyState.getState() != AuthProtocolState.UNCHALLENGED) return;
        AuthScheme authScheme2 = authCache.get(proxy);
        if (authScheme2 == null) return;
