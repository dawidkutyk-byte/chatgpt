...
extends AuthSchemeBase {
    ...
    protected void parseChallenge(CharArrayBuffer buffer, int beginIndex, int endIndex) throws MalformedChallengeException {
        ...
        if (this.challenge.isEmpty()) {
            ...
            if (this.state.compareTo((Enum)State.MSG_TYPE1_GENERATED) < 0) {
                this.state = State.FAILED;
                throw new MalformedChallengeException("Out of sequence NTLM response message");
            }
            if (this.state != State.MSG_TYPE1_GENERATED) return;
    ...
    public Header authenticate(Credentials credentials, HttpRequest request) throws AuthenticationException {
        ...
        if (this.state == State.CHALLENGE_RECEIVED) {
            ...
            if (this.state != State.MSG_TYPE2_RECEVIED) throw new AuthenticationException("Unexpected state: " + this.state);
