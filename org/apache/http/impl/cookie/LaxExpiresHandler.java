...
implements CommonCookieAttributeHandler {
    ...
    private void skipDelims(CharSequence buf, ParserCursor cursor) {
        ...
        for (int i = indexFrom; i < indexTo && DELIMS.get(current = buf.charAt(i)); ++pos, ++i) {
        }
    ...
    public void parse(SetCookie cookie, String value) throws MalformedCookieException {
        ...
        if (!foundTime) throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        if (!foundDayOfMonth) throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        if (!foundMonth) throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        if (!foundYear) {
            throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        }
        if (year >= 70 && year <= 99) {
        ...
        if (day < true) throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        if (day > 31) throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        if (year < 1601) throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        if (hour > 23) throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        if (minute > 59) throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        if (second > 59) {
            throw new MalformedCookieException("Invalid 'expires' attribute: " + value);
        }
