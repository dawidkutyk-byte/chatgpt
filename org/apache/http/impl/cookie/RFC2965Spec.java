...
extends RFC2109Spec {
    ...
    public RFC2965Spec(String[] datepatterns, boolean oneHeader) {
        super(oneHeader, new CommonCookieAttributeHandler[]{new RFC2965VersionAttributeHandler(), new /* Unavailable Anonymous Inner Class!! */, new RFC2965DomainAttributeHandler(), new RFC2965PortAttributeHandler(), new BasicMaxAgeHandler(), new BasicSecureHandler(), new BasicCommentHandler(), new BasicExpiresHandler(datepatterns != null ? (String[])datepatterns.clone() : DATE_PATTERNS), new RFC2965CommentUrlAttributeHandler(), new RFC2965DiscardAttributeHandler()});
    }
    ...
    private List<Cookie> createCookies(HeaderElement[] elems, CookieOrigin origin) throws MalformedCookieException {
        ...
        while (i$ < len$) {
            ...
            cookie.setPath(RFC2965Spec.getDefaultPath((CookieOrigin)origin));
            cookie.setDomain(RFC2965Spec.getDefaultDomain((CookieOrigin)origin));
