...
implements CommonCookieAttributeHandler {
    ...
    public boolean match(Cookie cookie, CookieOrigin origin) {
        ...
        if (i >= 0) {
            ...
            if (this.localDomainMap.containsKey(domain)) return this.handler.match(cookie, origin);
            if (!this.publicSuffixMatcher.matches(host)) return this.handler.match(cookie, origin);
            return false;
        }
        if (host.equalsIgnoreCase(origin.getHost())) return this.handler.match(cookie, origin);
        if (!this.publicSuffixMatcher.matches(host)) return this.handler.match(cookie, origin);
        return false;
    }
    ...
    public PublicSuffixDomainFilter(CommonCookieAttributeHandler handler, PublicSuffixList suffixList) {
        ...
        this.publicSuffixMatcher = new PublicSuffixMatcher((Collection)suffixList.getRules(), (Collection)suffixList.getExceptions());
