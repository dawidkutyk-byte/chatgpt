...
public class NetscapeDraftHeaderParser {
    ...
    private NameValuePair parseNameValuePair(CharArrayBuffer buffer, ParserCursor cursor) {
        ...
        if (cursor.atEnd()) return new BasicNameValuePair(name, value);
        cursor.updatePos(cursor.getPos() + 1);
        return new BasicNameValuePair(name, value);
    }

    static {
        TOKEN_DELIMS = TokenParser.INIT_BITSET((int[])new int[]{61, 59});
        VALUE_DELIMS = TokenParser.INIT_BITSET((int[])new int[]{59});
    }
