...
implements ClientExecChain {
    ...
    static void rewriteRequestURI(HttpRequestWrapper request, HttpRoute route, boolean normalizeUri) throws ProtocolException {
        try {
            ...
            if (uri == null) return;
            uri = uri.isAbsolute() ? URIUtils.rewriteURI((URI)uri, null, (EnumSet)(normalizeUri ? URIUtils.DROP_FRAGMENT_AND_NORMALIZE : URIUtils.DROP_FRAGMENT)) : URIUtils.rewriteURI((URI)uri);
            request.setURI(uri);
        }
    ...
    public CloseableHttpResponse execute(HttpRoute route, HttpRequestWrapper request, HttpClientContext context, HttpExecutionAware execAware) throws IOException, HttpException {
        ...
        try {
            ...
            if (entity != null) {
                if (entity.isStreaming()) return new HttpResponseProxy(response, releaseTrigger);
            }
