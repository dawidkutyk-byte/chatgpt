...
implements ClientExecChain {
    ...
    public CloseableHttpResponse execute(HttpRoute route, HttpRequestWrapper request, HttpClientContext context, HttpExecutionAware execAware) throws HttpException, IOException {
        ...
        while (true) {
            ...
            try {
                if (!this.retryStrategy.retryRequest((HttpResponse)response, c, (HttpContext)context)) return response;
                if (!RequestEntityProxy.isRepeatable((HttpRequest)request)) return response;
                response.close();
