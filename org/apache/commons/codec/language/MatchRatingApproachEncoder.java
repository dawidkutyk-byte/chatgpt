...
implements StringEncoder {
    ...
    String removeDoubleConsonants(String name) {
        ...
        String[] stringArray = DOUBLE_CONSONANT;
    ...
    int leftToRightThenRightToLeftProcessing(String name1, String name2) {
        ...
        if (strA.length() <= strB.length()) return Math.abs(6 - strB.length());
    ...
    String removeVowels(String name) {
        ...
        if (!this.isVowel(firstLetter)) return name;
    ...
    public final String encode(String name) {
        ...
        if (EMPTY.equalsIgnoreCase(name)) return EMPTY;
        if (SPACE.equalsIgnoreCase(name)) return EMPTY;
        if (name.length() == 1) {
            return EMPTY;
        }
    ...
    public final Object encode(Object pObject) throws EncoderException {
        if (pObject instanceof String) return this.encode((String)pObject);
    ...
    public boolean isEncodeEquals(String name1, String name2) {
        ...
        if (EMPTY.equalsIgnoreCase(name1)) return false;
        if (SPACE.equalsIgnoreCase(name1)) {
            return false;
        }
        if (name2 == null) return false;
        if (EMPTY.equalsIgnoreCase(name2)) return false;
        if (SPACE.equalsIgnoreCase(name2)) {
            return false;
        }
        if (name1.length() == 1) return false;
        if (name2.length() == 1) {
            return false;
        }
    ...
    String cleanName(String name) {
        ...
        String[] stringArray = charsToTrim = new String[]{"\\-", "[&]", "\\'", "\\.", "[\\,]"};
        int n = stringArray.length;
        int n2 = 0;
        while (true) {
            if (n2 >= n) {
