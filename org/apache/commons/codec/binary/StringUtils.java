...
public class StringUtils {
    public static boolean equals(CharSequence cs1, CharSequence cs2) {
        ...
        if (!(cs1 instanceof String) || !(cs2 instanceof String)) return cs1.length() == cs2.length() && CharSequenceUtils.regionMatches(cs1, false, 0, cs2, 0, cs1.length());
    ...
    private static byte[] getBytes(String string, Charset charset) {
        if (string != null) return string.getBytes(charset);
        return null;
    }

    private static ByteBuffer getByteBuffer(String string, Charset charset) {
        if (string != null) return ByteBuffer.wrap(string.getBytes(charset));
        return null;
    }
