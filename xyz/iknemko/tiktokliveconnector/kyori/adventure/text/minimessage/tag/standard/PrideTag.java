...
extends GradientTag {
    ...
    static final TagResolver RESOLVER = TagResolver.resolver((String)"pride", PrideTag::create);
    ...
    public Stream<? extends ExaminableProperty> examinableProperties() {
        return Stream.of(ExaminableProperty.of((String)"flag", (String)this.flag), ExaminableProperty.of((String)"phase", (double)this.phase));
    }

    static Tag create(ArgumentQueue args, Context ctx) {
        double phase = 0.0;
        String flag = PRIDE;
        if (!args.hasNext()) return new PrideTag(phase, FLAGS.get(flag), flag, ctx);
        ...
        if (value.isEmpty()) return new PrideTag(phase, FLAGS.get(flag), flag, ctx);
        try {
        ...
        if (phase < -1.0) throw ctx.newException(String.format("Gradient phase is out of range (%s). Must be in the range [-1.0, 1.0] (inclusive).", phase), args);
        if (!(phase > 1.0)) return new PrideTag(phase, FLAGS.get(flag), flag, ctx);
    ...
    public boolean equals(@Nullable Object other) {
        ...
        PrideTag that = (PrideTag)((Object)other);
