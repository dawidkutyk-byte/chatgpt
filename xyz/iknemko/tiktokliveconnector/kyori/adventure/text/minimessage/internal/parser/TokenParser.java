 ...
 *  xyz.iknemko.tiktokliveconnector.kyori.adventure.text.minimessage.internal.parser.TokenParser$1
 ...
 *  xyz.iknemko.tiktokliveconnector.kyori.adventure.text.minimessage.internal.parser.match.StringResolvingMatchedTokenConsumer
 *  xyz.iknemko.tiktokliveconnector.kyori.adventure.text.minimessage.internal.parser.match.TokenListProducingMatchedTokenConsumer
...
import xyz.iknemko.tiktokliveconnector.kyori.adventure.text.minimessage.internal.parser.match.StringResolvingMatchedTokenConsumer;
import xyz.iknemko.tiktokliveconnector.kyori.adventure.text.minimessage.internal.parser.match.TokenListProducingMatchedTokenConsumer;
...
public final class TokenParser {
    ...
    private static void parseSecondPass(String message, List<Token> tokens) {
        ...
        while (iterator.hasNext()) {
            ...
            block9: for (int i = startIndex; i < endIndex; ++i) {
                ...
                if (!escaped) {
                    if (codePoint == 92 && i + 1 < message.length()) {
                        ...
                        switch (1.$SwitchMap$net$kyori$adventure$text$minimessage$internal$parser$TokenParser$SecondPassState[state.ordinal()]) {
                            case 1: {
            ...
            int end = ((Token)token.childTokens().get(token.childTokens().size() - 1)).endIndex();
    ...
    public static List<Token> tokenize(String message, boolean lenient) {
        ...
        List tokens = listProducer.result();
    ...
    public static void parseString(String message, boolean lenient, MatchedTokenConsumer<?> consumer) {
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (!escaped) {
                if (codePoint == 92 && i + 1 < message.length()) {
                    int nextCodePoint = message.codePointAt(i + 1);
                    switch (1.$SwitchMap$net$kyori$adventure$text$minimessage$internal$parser$TokenParser$FirstPassState[state.ordinal()]) {
                        case 1: {
                            escaped = nextCodePoint == 60 || nextCodePoint == 92;
                            break;
                        }
    ...
    private static RootNode buildTree(@NotNull TagProvider tagProvider, @NotNull Predicate<String> tagNameChecker, @NotNull List<Token> tokens, @NotNull String message, @NotNull String originalMessage, boolean strict) throws ParsingException {
        RootNode root;
        RootNode node = root = new RootNode(message, originalMessage);
        for (Token token : tokens) {
            TokenType type = token.type();
            switch (1.$SwitchMap$net$kyori$adventure$text$minimessage$internal$parser$TokenType[type.ordinal()]) {
                case 1: {
                    node.addChild((ElementNode)new TextNode((ElementNode)node, token, message));
                    break;
                }
                case 2: 
                case 3: {
                    ...
                    if (!TagInternals.sanitizeAndCheckValidTagName((String)tagName)) {
                        node.addChild((ElementNode)new TextNode((ElementNode)node, token, message));
                        break;
                    }
                    TagNode tagNode = new TagNode((ElementNode)node, token, message, tagProvider);
                    if (tagNameChecker.test(tagNode.name())) {
                        ...
                        tagNode.tag(tag);
                        node.addChild((ElementNode)tagNode);
                ...
                case 4: {
                    ...
                    for (Token childToken : childTokens) {
                        closeValues.add(TagPart.unquoteAndEscape((String)message, (int)childToken.startIndex(), (int)childToken.endIndex()));
                    }
                    ...
                    RootNode parentNode = node;
                    while (parentNode instanceof TagNode) {
                        List openParts = ((TagNode)parentNode).parts();
                        if (TokenParser.tagCloses(closeValues, openParts)) {
                            ...
                            if (par == null) throw new IllegalStateException("Root node matched with close tag value, this should not be possible. Original text: " + message);
                            node = par;
                            break;
                        }
        ...
        if (!strict) return root;
        if (root == node) return root;
        ArrayList<TagNode> openTags = new ArrayList<TagNode>();
        for (RootNode n = node; n != null && n instanceof TagNode; n = n.parent()) {
    ...
    public static String resolvePreProcessTags(String message, TagProvider provider) {
        ...
        do {
            ...
            if (++passes >= 16) return lastResult;
    ...
    public static String unescape(String text, int startIndex, int endIndex, IntPredicate escapes) {
        ...
        if (i == -1) return text.substring(from, endIndex);
        if (i >= endIndex) {
